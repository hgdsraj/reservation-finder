...
describe('normalizeVenue', () => {
  ...
  test('extracts time slots with correct time and booking URL', () => {
    ...
    // Slots link to the venue page with date + party preset
    expect(r.slots[0].url).toContain('le-bernardin-new-york');
    expect(r.slots[0].url).toContain('date=2026-07-01');
    expect(r.slots[0].url).toContain('seats=2');
  });

  test('slot URLs avoid the dead book/details deeplink even when a token exists', () => {
    const raw = {
      ...RAW_VENUE,
      slots: [{ date: { start: '2026-07-01 19:00:00' }, config: { token: 'rgs://resy/123/456' } }],
    };
    const r = normalizeVenue(raw, '2026-07-01', 4);
    expect(r.slots[0].url).not.toContain('/book/details');
    expect(r.slots[0].url).toContain('le-bernardin-new-york');
    expect(r.slots[0].url).toContain('seats=4');
  });
